...
def load_state() -> dict:
    ...
    try:
        with open(STATE_FILE, "r") as f:
            state = json.load(f)
        # Backfill any missing keys (e.g. after adding new fields later)
...
def _open_long(exchange, amount, params):
    # 'buy' with no reduceOnly = open/increase a long position on a futures contract
    return exchange.create_order(SYMBOL, "market", "buy", amount, params=params)


@with_retries()
def _close_long(exchange, amount, params):
    # 'sell' with reduceOnly = True closes an existing long without opening a short
